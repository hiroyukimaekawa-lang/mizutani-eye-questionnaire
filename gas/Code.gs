...
function doPost(e) {
  try {
    ...
    var medical = Number(data.medicalCareRating);
    var staff = Number(data.staffRating);
    if (!Number.isFinite(medical) || medical < 1 || medical > 10 || !Number.isFinite(staff) || staff < 1 || staff > 10) {
    ...
    sheet.appendRow([
      ...
      String(data.gender || ''), String(data.ageGroup || ''), medical, staff,
      medical + staff, Number(((medical + staff) / 2).toFixed(2)), reasons,
