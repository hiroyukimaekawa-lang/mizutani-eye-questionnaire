...
export async function POST(request: Request) {
  try {
    ...
    if (!payload || payload.clinicKey !== 'mizutani') {
    ...
    try {
      ...
      // Preserve the raw response for server-side diagnostics without exposing it to the client.
    ...
    return Response.json({ ok: true, clinic: gasBody.clinic || 'mizutani' });
  } catch (error) {
