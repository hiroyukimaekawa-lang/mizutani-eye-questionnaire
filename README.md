...
Google Apps Script（GAS）のURLが未設定でも、フォーム入力、必須チェック、スコア計算、サンクス画面までローカルで確認できます。Google口コミボタンは口コミURLを設定した場合だけ表示し、回答点数による表示・非表示の切り替えは行いません。
...
現在はどちらも未設定です。確認できていないURLや架空のURLは入力しないでください。変更後は開発サーバーを再起動します。
...
`回答日時 / 性別 / 年代 / 診療内容満足度 / スタッフ対応満足度 / 合計スコア / 平均スコア / 当院を選んだ理由 / その他理由 / 自由記述`

空のシートでは、最初の回答時にヘッダーが自動作成されます。GASコードを更新した場合は、新しいバージョンとして再デプロイしてください。

## Google口コミURL設定

水谷眼科診療所が管理するGoogle Business Profileから正しい口コミ依頼リンクを取得し、`NEXT_PUBLIC_GOOGLE_REVIEW_URL` に設定します。URL未設定時は、サンクス画面に口コミ案内自体を表示しません。
